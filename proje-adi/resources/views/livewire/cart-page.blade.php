<div class="w-full max-w-[85rem] py-10 px-4 sm:px-6 lg:px-8 mx-auto">
    ...
    <tr wire:key="{{ $item['product_id'] }}">
        ...
        <td class="py-4">
            <div class="flex items-center">
                <button wire:click='azalanbtn({{$item['product_id']}})' class="px-4 py-2 mr-2 border rounded-md">-</button>
                <span class="w-8 text-center">{{$item['quantity']}}</span>
                <button wire:click='artanbtn({{$item['product_id']}})'class="px-4 py-2 ml-2 border rounded-md">+</button>
            </div>
        </td>
        <td class="py-4">{{Number::currency($item['total_amount'])}}</td>
        <td>
            <button wire:click='removeItem({{$item['product_id']}})'class="px-3 py-1 border-2 rounded-lg bg-slate-300 border-slate-400 hover:bg-red-500 hover:text-white hover:border-red-700"><span>Sil</span></button>
        </td>
    </tr>
@empty
    <tr>
        <td colspan="5" class="text-center py-4 text-4x1 font-semibold text-slate-500">Sepette ürün yok</td>
    </tr>
@endforelse
        ...
        <div class="md:w-1/4">
          <div class="p-6 bg-white rounded-lg shadow-md">
            ...
            @if ($cart_items)
            <a href="/checkout" class="w-full px-4 py-2 mt-4 text-white bg-blue-500 block text-center rounded-lg">kontrol et</a>
            @endif
